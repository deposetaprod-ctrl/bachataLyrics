...
DECLARE
    my_user_id UUID;
BEGIN
    -- 1. On cherche l'ID de l'utilisateur à partir de son email
    SELECT id INTO my_user_id 
    FROM auth.users 
    WHERE email = 'maximilien.godeau.off@gmail.com' 
    LIMIT 1;

    -- Si l'email n'est pas trouvé, on arrête le script
    IF my_user_id IS NULL THEN
        RAISE EXCEPTION 'Utilisateur introuvable avec cet email.';
    END IF;

    -- 2. On insère les notes pour cet utilisateur
